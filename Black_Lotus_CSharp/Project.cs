...
namespace Black_Lotus_CSharp
{
    public partial class Project : Form
    {
        ...
        DataTable table;
        MySqlConnection Connection = new MySqlConnection("datasource = localhost;port = 3306;Initial Catalog = 'wfm-cms';username = root;password =");
        ...
        {
            Connection.Open();
            String query = "SELECT * FROM client WHERE id =" + int.Parse(bunifuMetroTextbox_scusid.Text);
            command = new MySqlCommand(query, Connection);
            mdr = command.ExecuteReader();

            if (mdr.Read())
            {
                bunifuMetroTextbox_sfname.Text = mdr.GetString("Name");
